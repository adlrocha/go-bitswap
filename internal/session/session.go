...
func (s *Session) broadcast(ctx context.Context, wants []cid.Cid) {
	// Relay sessions do not broadcast when the relay session is exhausted. This
	// session is permanently started and should only react to messages from other peers,
	// not actively search for new blocks.
	if !s.relay {
...
func (s *Session) handlePeriodicSearch(ctx context.Context) {
	// Periodic searches only apply to direct sessions.
	if !s.relay {
